...
N_MODES = 512                    # Numero di basi da estrarre (p)
VISUALIZE = True
...
if VISUALIZE:
    print("Visualizing first 3 modes...")
    ...
    # Plottiamo il modo 0, 1 e 2 al tempo t=Nt/2 (metà simulazione)
    t_idx = Nt // 2
    
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    for i in range(3):
        mode_slice = modes_reshaped[:, :, t_idx, i]
        im = axes[i].imshow(mode_slice, cmap='seismic', origin='lower')
        axes[i].set_title(f"SVD Mode {i} (at t={t_idx})")
        plt.colorbar(im, ax=axes[i])
    
    plt.suptitle(f"Trunk Basis Functions ({actual_modes} modes captured)")
    plt.savefig(os.path.join(SCRIPT_DIR, 'data/svd_modes_check.png'))
    plt.show()
