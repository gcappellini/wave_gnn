...
class FFTTimePDE(dde.data.TimePDE):
    """Custom TimePDE - not needed, using standard TimePDE instead."""
    pass


class FFTWrapper(torch.nn.Module):
    """Wrapper that applies FFT transform before FNN and handles gradients correctly."""
    
    def __init__(self, fnn_net, feature_transform):
        super().__init__()
    ...
    def forward(self, x):
        """
        x: physical coordinates (batch, 2) with requires_grad for PDE computation
        Returns: network output in physical space
        """
    ...
    def parameters(self, recurse=True):
        ...
        return self.fnn_net.parameters(recurse)
    
    def named_parameters(self, prefix='', recurse=True):
        """Only return FNN parameters, not FFT buffers."""
        return self.fnn_net.named_parameters(prefix, recurse)
    
    def __getattr__(self, name):
        """Forward attribute access to fnn_net for DeepXDE compatibility."""
        try:
            return super().__getattr__(name)
        except AttributeError:
            return getattr(self.fnn_net, name)
...
if __name__ == "__main__":
    ...
    if apply_fft:
        # Network operates in spectral domain
        # FFT output: 2*m_spatial + 2*len(sigma_temporal_list)*m_temporal
        fft_output_dim = feature_transform.get_output_dim()
        layer_size = [fft_output_dim] + [hidden_feats] * hidden_layers + [1]
        print(f"Using FFT features: input dimension = {fft_output_dim}")
    else:
        # Network operates in physical domain
        layer_size = [2] + [hidden_feats] * hidden_layers + [1]
        print(f"Not using FFT features: input dimension = 2")
    ...
    # Add FFT-specific parameters only if FFT is enabled
    if apply_fft:
        fft_params = {
            'm_spatial': m_spatial,
            'm_temporal': m_temporal,
        ...
        # Add sigma_temporal values dynamically based on list length
        for i, sigma_val in enumerate(sigma_temporal_list):
            fft_params[f'sigma_temporal_{i}'] = sigma_val
        
        params.update(fft_params)
